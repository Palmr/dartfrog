...
$(function () {
  ...
  $('#editor').w2layout({
    name: 'editor',
    panels: [
      { type: 'main', content: '<button onclick="javascript:runStatementUnderCursor();">Run?</button><button onclick="javascript:getTableMetaData();">tablemetadata</button><textarea id="code">select * from portalmgr.web_roles</textarea>',  resizable: true, size: 50},
  ...
  w2ui['sidebar'].content('main',$().w2sidebar({
    ...
    onClick: function (event) {
      ...
      runQuery("SELECT * FROM " + tableName, function(results){
  ...
  w2ui['schema'].content('main', $().w2grid({
    ...
    onClick: function(event) {
      console.log(event);
      console.log(currentTableMetadata.columns[event.column]);
    }
  }));
});
